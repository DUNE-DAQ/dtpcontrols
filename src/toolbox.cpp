...
//-----------------------------------------------------------------------------
bool
source_sink_exists(const std::string& name, std::map<std::string, uint32_t>& map)
{
...
//-----------------------------------------------------------------------------
bool
source_sink_exists(const uint32_t& name, const std::vector<uint32_t>& mux_values)
{
...
//-----------------------------------------------------------------------------
std::vector<std::uint64_t>
read_WIB_pattern_from_file(std::string& path)
{
  ...
  // each pattern-file line is of example form '0x00554a00 1 0 0 1'
    ...
    while (std::getline(pattern_file_stream, pattern_line)) {
      ...
      pattern_data.push_back(tokens.at(0) + (tokens.at(1) << 32) + (tokens.at(2) << 33) + (tokens.at(3) << 34) +
                             (tokens.at(4) << 35));
    }
    return pattern_data;
  }
